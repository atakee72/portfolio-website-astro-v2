...
- **Security headers via `vercel.json`** — sitewide HTTP headers live under the `headers` array in `vercel.json`, not in `BaseLayout` meta tags. Current: `Referrer-Policy: strict-origin-when-cross-origin` (strips path from referer sent to third parties like Cloudinary, GoatCounter). When adding more (CSP, X-Frame-Options, etc.), append to the same `headers` block with the same `"source": "/(.*)"` matcher. See [[csp-deferred]] for why CSP isn't there yet.
