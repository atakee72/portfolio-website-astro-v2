...
- **Cloudinary transform ORDER decides whether the watermark survives — and getting it wrong fails silently with a 200.** `getCldImageUrl` emits `rawTransformations` first, then `overlays`, then any `width`/`height`/`crop` you pass. So a top-level `crop: 'fill', gravity: 'auto'` runs *after* the overlay: the © ATAK gets stamped bottom-right on the full 2000px original, then `g_auto` crops to the subject and discards that corner. The URL still contains `l_text`, the fetch still returns HTTP 200, the JPEG is still valid — **the only tell is that the bytes are identical to the unstamped version** (94069 vs 94753 on the roll used to find this, 2026-08-15). Put the crop in the raw list so it runs before the stamp; `src/lib/cloudinary.ts` `ogImageUrl()` does this and is the only place that should build a social card. **Verify this class of change by downloading the image and looking at it** — every automated check passes.
- **The watermark and metadata strip are defined ONCE** in `src/lib/cloudinary.ts` (`WATERMARK_OVERLAYS`, `STRIP_METADATA`), imported by `CloudPhoto.astro` and `ogImageUrl()`. They used to be inline in `CloudPhoto` only, which is exactly why og:images shipped unwatermarked for months — a second code path that nobody remembered was a second code path. Don't re-inline them.
...
- **Refresh workflow (proven three times — Jul 2026 case-study wave, Aug 2026 Vibes, Aug 2026 Coachly)**: read-only recon agent surveys the GitHub repo via `gh` (commits since the case study's date, README, `package.json`, file tree, any repo-side CLAUDE.md/ROADMAP) and writes a report file; I then write from the report + my own spot-checks.
- **When counts have drifted, make the recon pin the baseline first**: find the past commit where the case study's published numbers all reproduce exactly (the Coachly recon did this — 18/21/14/7 all matched file counts at one commit). That one move validates the interpretation ("18 tests" = 18 *files*, not assertions) AND proves the old text was accurate when written, so the new counts can use the same counting rule instead of a guess. **Also search Fabric for that project's notes** — the repo tells you *what* shipped, the Fabric gotcha notes tell you *why* a decision was made, which is the material that actually makes a case study worth reading (e.g. Vibes' hand-built modals exist because `prompt()`/`alert()`/`confirm()` interrupt media playback).
...
- **Opinion posts get REQUIRED READING, not a fact list** (proven on the stack-rationale post, Aug 2026): the brief names Ercan's own published posts/pages as the only permitted opinion source ("an opinion he never published is an ADDITION") and the report contract gains a SOURCES MAP — one line per section naming its grounding. Two review heuristics from that round: **(1) writers bend facts toward narrative symmetry** — when a draft sorts items under a tidy rule, check every item against the source table (DigiScrape got misfiled onto the SvelteKit side because the rule read better that way; "six clean calls, one hybrid" was both true and stronger); **(2) recon hedges are load-bearing** — "apparently hit in practice" must stay a hedge in the brief; upgrading it to "I hit this" shipped a false personal claim on the Meta post that Ercan had to correct (fixed `3f087e0`). Verify lived-experience claims with him before publishing them under his name.
...
- **Look at every capture before wiring it.** Screenshotting a live app surfaces whatever is actually in it: the MaHalle Forum and Markt shots came back full of keyboard-mash test posts (`hehjehej`, `dfjaksfas asdfasdf`), which reads as an unfinished product on a portfolio. Those two were held back and flagged rather than published. (Resolved Aug 2026: the junk was purged from prod by hand and a clean forum shot shipped — but note the purge was a manual DB action with no repo trace, so never describe it as shipped code in a case study.)
- **Login-gated surfaces can't be self-shot.** MaHalle's member pages (incl. `/forum`) sit behind a login gate since the Aug 2026 landing release — Ercan supplies those screenshots (1440×900, 2×); only public pages (`/`, marketplace, blog) can be captured headlessly.
...
- **Scheduled posts**: `getAllBlogPosts()` hides future-dated posts in prod (visible in dev, like drafts). `.github/workflows/rebuild-daily.yml` curls a Vercel Deploy Hook daily at 05:00 UTC so a post dated `YYYY-MM-DD` goes live that morning (~07:00 Berlin) without a push — any ordinary push also publishes due posts, since the filter runs at build time (edge: a push between 00:00 and 02:00 Berlin summer time won't publish a post dated that same day yet — it's not 00:00 UTC yet; the 05:00 UTC cron catches it the same morning). **GitHub cron is best-effort, sometimes VERY late**: the 05:00 UTC run usually fires ~05:20, but on 2026-08-27/28 it fired at 15:56 and 16:57 UTC (still `event: schedule`, still green) — a queued post can go live in the late afternoon instead of the morning. If a post absolutely must be live by a deadline, push any commit (or curl the deploy hook) that morning instead of trusting the cron slot. Date-only frontmatter = UTC midnight. The hook URL lives in the repo secret `DEPLOY_HOOK_URL` (Vercel dashboard → Settings → Git → Deploy Hooks, hook name `daily-scheduled-posts`, branch `main`). GitHub disables cron workflows after 60 quiet days — queueing a post resets the clock, but a post dated >60 days out needs a manual re-enable (Actions tab) near its date. Note: Vercel *preview* deployments are also production builds, so scheduled posts are hidden there too — preview a future post with `pnpm dev`.
...
- **Gotcha — `og:image` must be ABSOLUTE, and the bug lives per-LAYOUT.** Every layout that passes `ogImage` to `BaseLayout` must resolve it against `Astro.site` first — a root-relative path renders as `content="/assets/…"`, which OG scrapers (Slack/X/WhatsApp) silently ignore. Bit TWICE: the museum post 2026-08-09 (`BlogPostLayout`, fixed 48fbdb1) and then the SAME bug sat unfixed in `ProjectPageLayout` until 2026-08-21 (`d466293`) — all seven `/work` pages shipped broken link previews for two weeks after the blog fix was documented here. Both layouts now do `new URL(src, Astro.site).href`. If you add a layout that emits `ogImage`, absolutize there too — and verify from outside: `curl -s <live-url> | grep -o 'og:image" content="[^"]*'`. Note blog `mainImage` is not meta-only: it renders as a visible hero in the post AND on the blog-index featured card — don't repeat it inline in the body.
...
- **Scheduler verdict (2026-08-28, 9 days after the first post): stays unbuilt.** The astro-migration post did 14 views / 0 reactions on dev.to and ~zero referral traffic to the site — normal for a zero-follower account, but no case for automation. The manifest-driven design (`crossposts.json` of article id + date, second Actions cron at 12:30 UTC, key as repo secret) is kept here as the blueprint; revisit after 2–3 more technical cross-posts if views reach the hundreds. Stats pull: `GET https://dev.to/api/articles/me` with the `api-key` header — dev.to 403s Python-urllib's default UA ("Forbidden Bots"), set any custom `User-Agent`; curl passes as-is.
...
- **State as of 2026-08-17**: pins = portfolio, MaHalle (`fullstack-community-webApp-astro---v.3`), museum, vibes, rescue-kit; bootcamp-era repos + MaHalle v1/v2 ancestors + old portfolio + `translation-office-ai-assistant` made **private** (`movie-db`, `Firefox_Deepl_Extension` deliberately kept public for further development); `CODAC-monorepo` (fork, unprivatizable) **deleted 2026-08-19** after rescuing its two unique branches (`ercans-main-branch` — 18 commits ahead of upstream — and `lms-integrated`) into the private repo `codac-bootcamp-work`; lesson: audit ALL fork branches with `compare` `ahead_by` before calling a fork empty, the default branch was clean but the work sat on side branches. Bio + website set; avatar is the **1-bit cyanotype dither** of the headshot (all variants in `C:\Users\atakee\Pictures\avatar-variants\`).
